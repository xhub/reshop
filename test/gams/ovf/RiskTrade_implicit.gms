...
objective.. cost =e=  sum(t, Cst(ut1(t))) - Dem(consume1)
   + (1-lambda)*sum(s, prob(s)*(yh(s)+yt(s)+yc(s)))
   + lambda*(CVaRt + CVaRh + CVaRc);
...
put empinfo 'OVF cvarup CVaRt yt ' theta /;
put empinfo 'OVF cvarup CVaRh yh ' theta/;
put empinfo 'OVF cvarup CVaRc yc ' theta/;
putclose empinfo;
...
cpobjdef..
  cpobj =e= pi1*consume1 - Dem(consume1)
   + (1-lambdaC)*sum(s,prob(s)*ycp(s)) + lambdaC*CVaRc
   + sum(s,tradeprice(s)*tradec(s));

cpcvar(s).. ycp(s) =e= pi2(s)*consume2(s)- Dem(consume2(s)) - tradec(s);

tpobjdef(t)..
  tpobj(t) =e= -pi1*ut1(t) + Cst(ut1(t))
   + (1-lambdaT(t))*sum(s,prob(s)*ytp(s,t)) + lambdaT(t)*CVaRtp(t)
   + sum(s,tradeprice(s)*tradet(s,t));

tpcvar(s,t).. ytp(s,t) =e= pi2(s)*(-ut2(s,t)) - tradet(s,t) + Cst(ut2(s,t));

hpobjdef(h)..
  hpobj(h) =e= -pi1*Util(uh1(h))
   +(1-lambdaH(h))*sum(s,prob(s)*yhp(s,h)) + lambdaH(h)*CVaRhp(h)
  + sum(s,tradeprice(s)*tradeh(s,h));
...
put / 'min ' cpobj;
put consume1;
loop(s, put consume2(s), ycp(s), tradec(s));
put CVaRc;
put cpobjdef ;
...
loop(h,
 ...
 loop(s, put x2(s,h) uh2(s,h) sh2(s,h) yhp(s,h) tradeh(s,h));
 put CVaRhp(h)
 put hpobjdef(h) dynamics1(h);
$ifthen not set IMPLICIT
 loop(s, put dynamics2(s,h) hpcvar(s,h));
$else
...
loop(t,
 put / 'min ' tpobj(t);
 put ut1(t);
 loop(s, put ut2(s,t) ytp(s,t) tradet(s,t) );
 put CVaRtp(t)
$ifthen not set IMPLICIT
...
put / 'OVF cvarup CVaRc ycp ' theta/;
loop(h, put / "OVF cvarup " CVaRhp(h);
loop(s, put yhp(s,h));
 put theta/;
)
loop(t, put / "OVF cvarup " CVaRtp(t);
loop(s, put ytp(s,t);)
 put theta/;
);
putclose empinfo /;
